...
5. 为 Week 25 冻结可复用的 Kubernetes/Kueue、硬件和故障时间线。
...
- LWS 管理 Pod group 的 identity、rollout 与故障生命周期；vLLM distributed executor 仍负责组内进程/rank，不因使用 LWS 而消失。
...
- 不引入 KubeRay、RayService、autoscaling 或 AIBrix；一个 Service 只暴露 leader/API Pod，worker 不作为独立模型副本接流量。
...
- `reports/week24.md`：matrix 结果、未满足条件、控制器边界和 Week 25 handoff。
...
| vLLM runtime | rendezvous、rank join、模型加载和 API serving | world size、rank mapping、worker join、service ready | 不决定集群配额或创建 LWS group |
...
| Day 7 | 1 h | 整理 lifecycle/scheduling 证据和 Week 25 公平对照输入 |
...
7. Week 25 比较 KubeRay 时，哪些 Kueue、硬件和 runtime 变量必须保持不变？
