...
第二十三周只补 vLLM native multiprocessing 多节点启动、DP、EP 和通信测量资料。TP/PP 与 distributed serving 的基础继续复用 Week 14，不重复收录相同 URL。
...
| 已有来源 | 本周只查什么 |
|---|---|
| [Week 14 references](week-14-references.md) #1 | 精读 Parallelism and Scaling 的 native multiprocessing 多节点示例，核对 `--nnodes`、`--node-rank`、`--master-addr` 与 worker `--headless`；同时复用 TP/PP 部署建议和通信排障，这是该 URL 的唯一收录位置 |
...
| 日期 | 阅读 | 对应任务 |
|---|---|---|
| Day 1 | 复用 Week 14 #1；新增 1 | TP/PP/DP/EP、head/worker CLI 与 runtime topology |
| Day 2 | 新增 2–3；复用 Week 4 #4/#8 | 通信、CLI 和环境 preflight |
| Day 3 | 复用 Week 14 #1 | Native multiprocessing A0/A1 跨节点 PP smoke |
...
3. `--nnodes`、`--node-rank`、`--master-addr` 与 worker `--headless` 分别约束什么，native multiprocessing 与 NCCL collective 又分别负责哪一层？
